...
func newLoop(t *testing.T, client inference.Client) (*Loop, *recordingPublisher, context.CancelFunc) {
	...
	l, err := newWithConfig(ctx, sessionID, loopID, Provenance{}, rec, runtimeConfig{Client: client, Model: testModel(), DrainTimeout: 200 * time.Millisecond})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	t.Cleanup(cancel)
...
func TestNew_Validation(t *testing.T) {
	...
	t.Run("missing client", func(t *testing.T) {
		t.Parallel()
		_, err := newWithConfig(ctx, sessionID, loopID, Provenance{}, noopPublisher{}, runtimeConfig{Model: testModel()})
		var ce *ConfigError
	...
	t.Run("nil publisher", func(t *testing.T) {
		t.Parallel()
		_, err := newWithConfig(ctx, sessionID, loopID, Provenance{}, nil, runtimeConfig{Client: &fakeLLM{}, Model: testModel()})
		var ce *ConfigError
...
func TestStepGranularityRollback(t *testing.T) {
	...
	t.Run("fail mid-turn keeps completed steps committed, discards only the in-flight step (no unpaired tool_use)", func(t *testing.T) {
		...
		loopID, _ := uuid.New()
		rec := &recordingPublisher{}
		l, err := newWithConfig(ctx, sessionID, loopID, Provenance{}, rec,
