...
func newFoldLoop(t *testing.T, client inference.Client, ts ToolSet) (*Loop, *recordingPublisher) {
	...
	l, err := newWithConfig(ctx, sessionID, loopID, Provenance{}, rec,
		runtimeConfig{Client: client, Model: testModel(), Tools: ts, DrainTimeout: 500 * time.Millisecond})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	return l, rec
...
func newFoldLoopWithAfterDrain(t *testing.T, client inference.Client, ts ToolSet, afterDrain func()) (*Loop, *recordingPublisher, context.CancelFunc) {
	...
	rec := &recordingPublisher{}
	l, err := newWithConfig(ctx, sessionID, loopID, Provenance{}, rec,
		runtimeConfig{Client: client, Model: testModel(), Tools: ts, DrainTimeout: 500 * time.Millisecond, afterDrain: afterDrain})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	return l, rec, cancel
