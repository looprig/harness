...
func resolveMode(bound loop.BoundDefinition, modeName loop.ModeName) (runtimeConfig, error) {
	...
	resolved := runtimeConfig{
		...
		Tools: ToolSet{Access: bound.Access(), Registry: mode.Tools, Middlewares: bound.Middlewares(), MaxToolIterations: limits.Iterations, MaxToolCallsPerTurn: limits.Calls, MaxParallelToolCalls: limits.Parallel, MaxToolResultBytes: limits.ResultBytes, MaxToolResultCaptureBytes: limits.CaptureBytes, ToolResultReaderBound: mode.ToolResultReaderBound()},
	}
