...
// SetSecurityLimit clamps the Session security limit to level, the ordinal upper
// bound on how permissive auto-approval may be (SPEC §8/§10.2). It is the JOURNALED,
// replayable path (never a bare setter): it appends a command.SetSecurityLimit to the
// audit intent log, APPLIES it to the session's live security limit ordinal, and emits a durable
// event.SecurityLimitChanged. Harness treats level as an ORDINAL ONLY (0 = most
// restrictive); the consumer maps it to a mode.
//
// The clamp takes effect on the NEXT permission Check: the checker (wired via
// tools.WithSecurityLimitPostures over the SAME source this session holds — see WithSecurityLimit /
// SecurityLimitSource) reads the live ordinal per Check, so a change is visible immediately. An
// already-spawned tool call keeps its spawn-time policy — the executor was built per-spawn
// and the security limit is read only at Check time, never re-read mid-spawn — so tightening the
// security limit never retro-confines an in-flight call.
//
...
//     memory security limit is the SAFE direction, so applying ahead of the durable record is
//     acceptable.
...
// emitted SecurityLimitChanged events reproduces the exact ordinal (last write wins). A
...
func (s *Session) SetSecurityLimit(ctx context.Context, level security.Level) error {
	...
	// proceeds (a lost record must never block an operator's security action). The security limit
	// is session-global, so the record is anchored to the active loop purely as audit
	// metadata — replay reconstructs the security limit from the emitted events, not this record.
	...
	changed := event.SecurityLimitChanged{Header: stamped, Level: effective}
...
// securityLimitState returns the Session's live security limit, default-minting it when nil. The
...
func (s *Session) securityLimitState() *security.Limit {
	if s.securityLimit == nil {
		s.securityLimit = security.New()
	}
	return s.securityLimit
}

// SecurityLimitSource returns the session's live security limit source (READ-only): the same
// ordinal SetSecurityLimit mutates. The composition root wires this into a permission
// checker via tools.WithSecurityLimitPostures (structural — *security.Limit satisfies both
// security.LimitSource and tools.SecurityLimitSource), so posture selection and this session's security limit
// never disagree. It is always non-nil for a constructed session. When the composition
// root injected its own source via WithSecurityLimit, this returns that SAME instance (the
// checker and the session already share it).
func (s *Session) SecurityLimitSource() security.LimitSource {
	return s.securityLimitState()
}
