...
| Criterion | Evidence |
| --- | --- |
| Safe request auto-approves once | `TestPermissionClassifierSafeCommandAutoApproved` (package `github.com/looprig/tests`, repo tests-permission-classifier). Re-run fresh, uncached (`LOOPRIG_LIVE_NETWORK=0 GOWORK=off go test -tags integration -race -count=1 -v ./...`) 2026-07-29/30: `--- PASS: TestPermissionClassifierSafeCommandAutoApproved (1.56s)`. |
| Human can answer during review | `TestPermissionReviewDoesNotDelayHumanResponse` (`internal/loopruntime`, harness-permission-classifier). Read directly: starts a review that is still blocked in flight, sends a human `ApproveToolCall`, and asserts it is routed within 500ms "while review was still in flight." |
| Human/classifier races resolve exactly once | `TestRespondFromClassifierAndHumanSimultaneousRaceExactlyOneWins` (`internal/sessionruntime`, harness-permission-classifier), part of the `review_race_test.go` suite (Task 15) that also covers human-first, classifier-first, duplicate-classifier, and post-close races each resolving to exactly one `GateResolved`. |
| Every non-allow leaves gate open | `TestRespondFromClassifierDriftDropsSilently` (`internal/sessionruntime`, harness-permission-classifier). Read directly: table test over basis/context/policy/security/observation drift; each subtest asserts `ListGates()` still shows the gate open — "every non-allow path preserves the human gate." Corroborated by `TestRespondFromClassifierAfterGateCloseIsStale` and `TestRespondFromClassifierDuplicateIsStale` in the same file. |
| Critical never auto-approves | Two independent layers, both re-run fresh and PASS: (1) `TestPermissionReviewDefaultPolicyMatrix` (`pkg/gate`, harness-permission-classifier) — exhaustive risk×authorization×recommendation matrix hardcoding `wantEligible := ... && risk != gate.ReviewRiskCritical && ...`; read directly. (2) `TestEvaluateCorpusHasNoCriticalFalseAllows` (`pkg/commandsafety`, classifiers) — corpus-level check; re-run fresh 2026-07-30 (`GOCACHE=/private/tmp/looprig-classifiers-go-cache GOFLAGS=-mod=vendor go test -count=1 -v -run TestEvaluateCorpusHasNoCriticalFalseAllows ./pkg/commandsafety/...`): `--- PASS: TestEvaluateCorpusHasNoCriticalFalseAllows (0.01s)`. |
| No persistent approval/rule/grant authority | Structural + tested: `respondFromClassifier` (`internal/sessionruntime/gates.go:1055`, harness-permission-classifier) takes no `ApprovalAction` parameter and always stamps `gate.ResponseSource{Kind: gate.ResponseFromClassifier}` into the ordinary Approve-only path — there is no code path for classifier code to reach `ApproveAlways`, `Deny`, or rule persistence. Proven by `TestRespondGateRejectsPublicClassifierSourceDuringRace` (public callers cannot forge classifier provenance) and `TestRespondFromClassifierMintsGrantsThroughEvaluatorLikeHuman` (grants mint only through the ordinary evaluator, not classifier-controlled), both `internal/sessionruntime`, harness-permission-classifier. |
| Evidence tools are read-only and bounded | Bounded: `TestEvidenceExecutionEnforcesRoundAndCallBoundsBeforeWork` and `TestEvidenceExecutionEnforcesExactResultAndAggregateByteBounds` (`internal/hustleruntime`, harness-permission-classifier). Read-only/no runtime authority: `TestEvidenceContainmentPolicyCarriesNoRuntimeAuthority` (`internal/hustleruntime`, harness-permission-classifier). |
| Basis/security/observation drift blocks allow | Same evidence as "every non-allow leaves gate open" above: `TestRespondFromClassifierDriftDropsSilently` (`internal/sessionruntime`, harness-permission-classifier) is a table test with explicit subtests for basis drift, context drift, policy drift, security drift, and observation drift (context+security ceiling both moved), each asserting the response is silently dropped, no `GateResolved` is emitted, and the gate stays open. Read directly. |
| Durable audit excludes sensitive review data | `TestPermissionReviewAdapterNeverLeaksSecretsIntoDurableEvents` and `TestPermissionReviewPayloadHasNoSensitiveFields` (`internal/sessionruntime` / `pkg/event`, harness-permission-classifier); corroborated end-to-end by `TestPermissionClassifierAuditTrailNeverLeaksReviewRationale` (`github.com/looprig/tests`, tests-permission-classifier), re-run fresh 2026-07-30: `--- PASS: TestPermissionClassifierAuditTrailNeverLeaksReviewRationale (1.59s)`. |
| Restore is human-only | `TestRestorePermissionGateRestoresOpenAndReviewFree` (`internal/sessionruntime`, harness-permission-classifier) — restored gates come back open and review-free. Corroborated by `TestReviewSessionBreakerTripMakesNewTurnHumanOnly` (circuit-breaker path) and by `TestRestoreSurvivesHostOwnedGates` / `TestRestoreSurvivesDanglingPermissionGate` / `TestRestoreSurvivesDanglingHostGates` (`github.com/looprig/tests`, tests-permission-classifier), all re-run fresh 2026-07-30 and PASS. |
| Classifiers combine conjunctively | `TestCombinePermissionAssessments` and `TestCombinePermissionAssessmentsRequiresCompleteRegisteredSet` (`pkg/gate`, harness-permission-classifier). |
