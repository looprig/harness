...
func TestValidateCommandInvalid(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "CancelDelegateRequest missing SessionID",
			cmd:       command.CancelDelegateRequest{Header: hdr, Coordinates: identity.Coordinates{LoopID: loop}, TargetCommandID: target},
			wantCmd:   command.CommandCancelDelegateRequest,
			wantField: command.FieldSessionID,
			wantRule:  command.RuleRequired,
		},
		{
			name:      "CancelDelegateRequest missing LoopID",
			cmd:       command.CancelDelegateRequest{Header: hdr, Coordinates: identity.Coordinates{SessionID: sess}, TargetCommandID: target},
			wantCmd:   command.CommandCancelDelegateRequest,
			wantField: command.FieldLoopID,
			wantRule:  command.RuleRequired,
		},
		{
			name:      "CancelDelegateRequest missing TargetCommandID",
			cmd:       command.CancelDelegateRequest{Header: hdr, Coordinates: identity.Coordinates{SessionID: sess, LoopID: loop}},
			wantCmd:   command.CommandCancelDelegateRequest,
			wantField: command.FieldTargetCommandID,
			wantRule:  command.RuleRequired,
		},
