// pkg/llm/codec/gemini/encode.go
package gemini
...
// BuildGenerateContentRequest converts a provider-neutral llm.Request into a
...
func BuildGenerateContentRequest(req llm.Request) (GenerateContentRequest, error) {
...
// EncodeRequest converts a provider-neutral llm.Request to a Gemini
...
func EncodeRequest(req llm.Request) ([]byte, error) {
...
// omitted).
func buildTools(tools []llm.Tool) []geminiTool {
	if len(tools) == 0 {
		return nil
	}
...
func buildGenerationConfig(s llm.Sampling, caps llm.Capabilities) *generationConfig {
	gc := &generationConfig{
...
func thinkingFor(e llm.Effort, caps llm.Capabilities) *thinkingConfig {
	if !caps.Thinking {
		return nil
	}
...
func thinkingBudget(e llm.Effort) (int, bool) {
	switch e {
	case llm.EffortLow:
		return 4096, true
