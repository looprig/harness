...
func (s *readServer) handleListSessions(w http.ResponseWriter, r *http.Request) {
	q := r.URL.Query()
...
func (s *readServer) handleStatus(w http.ResponseWriter, r *http.Request) {
...
func (s *readServer) handleJournal(w http.ResponseWriter, r *http.Request) {
	sid, err := parseSessionID(r.PathValue("sid"))
	if err != nil {
		writeErrorCause(w, http.StatusBadRequest, codeInvalidParam, msgInvalidSID, false, err)
		return
