...
func FuzzPermissionReviewSubjectWire(f *testing.F) {
	...
	f.Add(bytes.Replace(valid, []byte(`"command":"git status"`), []byte(`"Command":"git status"`), 1))
	f.Add(bytes.Replace(valid, []byte(`"command":"git status"`), []byte(`"command":"git status","Command":"git status"`), 1))
	f.Add(bytes.Replace(valid, []byte(`"origin":"user"`), []byte(`"Origin":"user"`), 1))
	f.Add(bytes.Replace(valid, []byte(`"applied":0`), []byte(`"Applied":0`), 1))
